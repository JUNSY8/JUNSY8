<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=12,20,6&height=220&section=header&text=Ashborn&fontSize=42&fontColor=ffffff&animation=fadeIn&fontAlignY=35&desc=Full%20Stack%20Developer%20·%20Laravel%20%2B%20Vue&descAlignY=58&descSize=16" />

[![Typing SVG](https://readme-typing-svg.demolab.com?font=Fira+Code&size=18&duration=3000&pause=1200&color=A78BFA&center=true&vCenter=true&width=560&lines=Backend+architecture+%26+CRUD+systems;Laravel+%2B+Vue+%7C+regulation-driven+logic;Python+automation+%26+desktop+tooling)](https://git.io/typing-svg)

</div>

<img width="100%" src="https://capsule-render.vercel.app/api?type=rect&color=0:8B5CF6,100:3B82F6&height=3" />

## About Me

I build backend systems for business-critical applications — currently focused on **HR/payroll platforms** with complex, regulation-driven logic. My work centers on translating legal/business rules (e.g. Colombian labor law) into clean, maintainable database and application architecture.

**Currently building:**
- 🔧 Overtime/surcharge configuration module (Laravel + Vue) — compliant with **Ley 2466 de 2025**
- 🐍 Python desktop tooling for browser automation and QA (Tkinter + Selenium)

<img width="100%" src="https://capsule-render.vercel.app/api?type=rect&color=0:3B82F6,100:8B5CF6&height=3" />

## Technical Focus

| Area | What I do |
|---|---|
| **Backend Architecture** | Catalog/master-table design patterns, CRUD module systems, Eloquent relationships, Form Request validation |
| **Domain Logic** | Encoding labor-law and compliance rules into configurable, auditable database structures |
| **Frontend** | Vue.js components consuming Laravel APIs, form-driven UIs for config-heavy modules |
| **Automation** | Selenium-based browser automation, desktop GUIs (Tkinter) for internal tooling |

### Stack

<div align="center">

![PHP](https://img.shields.io/badge/PHP-777BB4?style=for-the-badge&logo=php&logoColor=white)
![Laravel](https://img.shields.io/badge/Laravel-FF2D20?style=for-the-badge&logo=laravel&logoColor=white)
![Vue.js](https://img.shields.io/badge/Vue.js-4FC08D?style=for-the-badge&logo=vuedotjs&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![Selenium](https://img.shields.io/badge/Selenium-43B02A?style=for-the-badge&logo=selenium&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)

</div>

<img width="100%" src="https://capsule-render.vercel.app/api?type=rect&color=0:8B5CF6,100:EC4899&height=3" />

## GitHub Stats

<div align="center">

<img src="https://github-readme-stats.vercel.app/api?username=ashborn8&show_icons=true&theme=tokyonight&hide_border=true&include_all_commits=true" width="48%" alt="GitHub Stats" />
<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=ashborn8&layout=compact&theme=tokyonight&hide_border=true" width="40%" alt="Top Languages" />

</div>

<img width="100%" src="https://capsule-render.vercel.app/api?type=rect&color=0:EC4899,100:F59E0B&height=3" />

## Featured Projects

> Reemplaza con tus 3 repos más fuertes — descripción tipo changelog, no adjetivos.

| Project | Description |
|---|---|
| **[project-name](https://github.com/ashborn8/repo)** | REST API for [X] with role-based access control and automated tests |
| **[project-name](https://github.com/ashborn8/repo)** | CRUD module generator for catalog/master tables in Laravel, used across N modules |
| **[project-name](https://github.com/ashborn8/repo)** | Selenium-based automation tool with configurable strategies and a Tkinter GUI |

<img width="100%" src="https://capsule-render.vercel.app/api?type=rect&color=0:F59E0B,100:10B981&height=3" />

<div align="center">

### Let's Connect

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/your-profile)
[![Gmail](https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white)](https://mail.google.com/mail/?view=cm&fs=1&to=your-email@gmail.com)

</div>

<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=12,20,6&height=90&section=footer" />
